...
const clean = {
  ...
  cleanCss: function () {
    return src(paths.css, { allowEmpty: true }, { read: false }).pipe(
      cleanModule()
    );
  },
  cleanJs: function () {
    return src(paths.jsOutput, { allowEmpty: true }, { read: false }).pipe(
      cleanModule()
    );
  },
  cleanDist: function () {
    return src(dirName.dist, { allowEmpty: true }, { read: false }).pipe(
      cleanModule()
    );
  },
  cleanPackage: function () {
    return src(dirName.package, { allowEmpty: true }, { read: false }).pipe(
      cleanModule()
    );
  },
};
...
function sprite() {
  var spriteData = src(`${paths.spriteIcons}/*.png`, { allowEmpty: true }).pipe(
    spritesmith({
...
function csso() {
  return src(`${paths.css}/**/*.css`, { allowEmpty: true })
    ...
    .pipe(
      renameModule({ extname: ".min.css" }).on("error", notifyModule.onError())
    )
    .pipe(sourcemaps.write("."))
    .pipe(dest(paths.css));
}
...
function jsConcat() {
  return src(
    [
      ...
      `${paths.jsLibs}/*.js`,
      `${paths.jsOutput}/*.js`,
      `!${paths.jsOutput}/app.js`,
      `!${paths.jsOutput}/app.min.js`,
    ],
    { allowEmpty: true }
  )
    .pipe(sourcemaps.init({ loadMaps: true }))
...
function jsMinify() {
  return src(`${paths.jsOutput}/app.js`, { allowEmpty: true })
    ...
    .pipe(
      renameModule({ extname: ".min.js" }).on("error", notifyModule.onError())
    )
    .pipe(sourcemaps.write("."))
    .pipe(dest(paths.jsOutput));
  //	.pipe(browserSync.stream())
...
function dist() {
  return src(
    [
      ...
      paths.ar,
      `${paths.app}/**/*`,
      `!${paths.assets}/{${dirName.sass},${dirName.sass}/**}`,
      `!${paths.js}/{${dirName.jsScripts},${dirName.jsScripts}/**}`,
      `!${paths.app}/{${dirName.views},${dirName.views}/**}`,
    ],
    { allowEmpty: true }
...
function packageTask() {
  return src(`${dirName.dist}/**/*`, { allowEmpty: true })
    .pipe(
      zipModule(`${dirName.package}_${utc}.zip`).on(
        "error",
        notifyModule.onError()
      )
    )
...
exports.packageProd = series(
  exports.runProd,
  dist,
  packageTask,
  clean.cleanDist
);
exports.packageToServer = series(
  exports.runProd,
  dist,
  packageServer,
  clean.cleanDist
